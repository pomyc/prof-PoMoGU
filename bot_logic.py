...
def calculate_relevance_score(result, query):
    ...
    if is_dues_query:
        ...
        if 'статут' in source:
            has_financial = any(term in content for term in high_value_terms + context_terms)
            if has_financial:
                score += 200  # Огромный бонус
            else:
                score += 10   # Небольшой бонус просто за статут
        
        # Бонус за фразы о размере взносов
        specific_phrases = [
...
def search_in_knowledge_base(query):
    ...
    try:
        ...
        # СТРОГАЯ фильтрация: исключаем заблокированные документы
        good_results = []
        for result, score in scored_results:
            if score > -500:  # Пропускаем только незаблокированные документы
                if score > 10:  # И с минимальным положительным баллом
                    good_results.append((result, score))
        
        print(f"📊 Всего результатов: {len(results)}, после фильтрации: {len(good_results)}")
        ...
        if vectorstore:
            try:
                ...
                if results:
                    ...
                    min_score = 15  # Снизили порог, но добавили предварительную фильтрацию
                    relevant_results = []
                    for result, score in scored_results:
                        if score > -500 and score >= min_score:  # Не заблокирован И релевантен
